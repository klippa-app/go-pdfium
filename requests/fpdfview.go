...
type FPDFBitmap_CreateEx struct {
	...
	Pointer any    // In the CGO runtime this must be an unsafe.Pointer to the first byte of a byte array, use unsafe.Pointer(&byteArray[0]) to get it. In the Webassembly runtime this must be uint64 with a pointer inside the Webassembly memory space created by malloc.
