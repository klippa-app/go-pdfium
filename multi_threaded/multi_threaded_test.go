...
var _ = BeforeSuite(func() {
	...
	pool := multi_threaded.Init(multi_threaded.Config{
		...
		Command: multi_threaded.Command{
			...
			Args:    []string{"run", "-tags", "pdfium_experimental", "../examples/multi_threaded/worker/main.go"}, // This is a reference to the worker package, this can be left empty when using a direct binary path.
		},
	})
