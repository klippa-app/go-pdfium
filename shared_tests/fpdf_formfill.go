...
var _ = Describe("fpdf_formfill", func() {
	...
	Context("a PDF file with a text form", func() {
		...
		renderFormImage := func(page references.FPDF_PAGE) {
			...
			os.WriteFile(TestDataPath+"/testdata/"+fmt.Sprintf("render_fpdf_formfill-%d.jpg", renderCount), imgBuf.Bytes(), 0777)
			renderCount++
	...
	Context("a normal PDF file", func() {
		...
		BeforeEach(func() {
			pdfData, err := os.ReadFile(TestDataPath + "/testdata/xfa_multiline_textfield.pdf")
			Expect(err).To(BeNil())
