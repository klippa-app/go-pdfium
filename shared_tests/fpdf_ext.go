...
func RunfpdfExtTests(pdfiumContainer pdfium.Pdfium, testsPath string, prefix string) {
	Describe("fpdf_ext", func() {
		...
		// These methods don't work on multi-threaded usage.
		if prefix == "multi" {
			return
		}
		...
		Context("the time function has been overwritten", func() {
			BeforeEach(func() {
				...
				resp, err := pdfiumContainer.FSDK_SetTimeFunction(&requests.FSDK_SetTimeFunction{
			...
			When("a new document is created", func() {
				It("returns the correct CreationDate date in the metadata", func() {
					newDoc, err := pdfiumContainer.FPDF_CreateNewDocument(&requests.FPDF_CreateNewDocument{})
					Expect(err).To(BeNil())
					Expect(newDoc).To(Not(BeNil()))

					meta, err := pdfiumContainer.GetMetaData(&requests.GetMetaData{
		...
		Context("the localtime function has been overwritten", func() {
			BeforeEach(func() {
				...
				resp, err := pdfiumContainer.FSDK_SetLocaltimeFunction(&requests.FSDK_SetLocaltimeFunction{
		...
		Context("the unsupported object processor function has been overwritten", func() {
			...
			BeforeEach(func() {
				...
				resp, err := pdfiumContainer.FSDK_SetUnSpObjProcessHandler(&requests.FSDK_SetUnSpObjProcessHandler{
