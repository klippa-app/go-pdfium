...
func RunfpdfSignatureTests(pdfiumContainer pdfium.Pdfium, testsPath string, prefix string) {
	Describe("fpdf_signature", func() {
		Context("no document is given", func() {
			It("returns an error when getting the document signature count", func() {
				FPDF_GetSignatureCount, err := pdfiumContainer.FPDF_GetSignatureCount(&requests.FPDF_GetSignatureCount{})
				Expect(err).To(MatchError("document not given"))
				Expect(FPDF_GetSignatureCount).To(BeNil())
			})

			It("returns an error when getting the document signature object", func() {
				FPDF_GetSignatureObject, err := pdfiumContainer.FPDF_GetSignatureObject(&requests.FPDF_GetSignatureObject{})
				Expect(err).To(MatchError("document not given"))
		...
		Context("no signature is given", func() {
			It("returns an error when getting the signature content", func() {
				FPDFSignatureObj_GetContents, err := pdfiumContainer.FPDFSignatureObj_GetContents(&requests.FPDFSignatureObj_GetContents{})
			...
			It("returns an error when getting the signature byte range", func() {
				FPDFSignatureObj_GetByteRange, err := pdfiumContainer.FPDFSignatureObj_GetByteRange(&requests.FPDFSignatureObj_GetByteRange{})
				Expect(err).To(MatchError("signature not given"))
				Expect(FPDFSignatureObj_GetByteRange).To(BeNil())
			})

			It("returns an error when getting the signature sub filter", func() {
				FPDFSignatureObj_GetSubFilter, err := pdfiumContainer.FPDFSignatureObj_GetSubFilter(&requests.FPDFSignatureObj_GetSubFilter{})
				Expect(err).To(MatchError("signature not given"))
				Expect(FPDFSignatureObj_GetSubFilter).To(BeNil())
			})

			It("returns an error when getting the signature reason", func() {
				FPDFSignatureObj_GetReason, err := pdfiumContainer.FPDFSignatureObj_GetReason(&requests.FPDFSignatureObj_GetReason{})
				Expect(err).To(MatchError("signature not given"))
				Expect(FPDFSignatureObj_GetReason).To(BeNil())
			})

			It("returns an error when getting the signature time", func() {
				FPDFSignatureObj_GetTime, err := pdfiumContainer.FPDFSignatureObj_GetTime(&requests.FPDFSignatureObj_GetTime{})
				Expect(err).To(MatchError("signature not given"))
				Expect(FPDFSignatureObj_GetTime).To(BeNil())
			})

			It("returns an error when getting the signature DocMDPPermission", func() {
				FPDFSignatureObj_GetDocMDPPermission, err := pdfiumContainer.FPDFSignatureObj_GetDocMDPPermission(&requests.FPDFSignatureObj_GetDocMDPPermission{})
				Expect(err).To(MatchError("signature not given"))
		...
		Context("a normal PDF file", func() {
			...
			When("is opened", func() {
				It("returns that it has no signatures", func() {
					signatureCount, err := pdfiumContainer.FPDF_GetSignatureCount(&requests.FPDF_GetSignatureCount{
						Document: doc,
					})
					Expect(err).To(BeNil())
		...
		Context("a PDF file with two signatures", func() {
			...
			When("the first signature object is opened", func() {
				...
				BeforeEach(func() {
					signatureResp, err := pdfiumContainer.FPDF_GetSignatureObject(&requests.FPDF_GetSignatureObject{
						Document: doc,
						Index:    0,
					})
					Expect(err).To(BeNil())
				...
				It("returns the correct signature content", func() {
					signatureContent, err := pdfiumContainer.FPDFSignatureObj_GetContents(&requests.FPDFSignatureObj_GetContents{
						Signature: signature,
					})
					Expect(err).To(BeNil())
				...
				It("returns the correct signature byte range", func() {
					signatureByteRange, err := pdfiumContainer.FPDFSignatureObj_GetByteRange(&requests.FPDFSignatureObj_GetByteRange{
						Signature: signature,
					})
					Expect(err).To(BeNil())
				...
				It("returns the correct signature sub filter", func() {
					subFilter, err := pdfiumContainer.FPDFSignatureObj_GetSubFilter(&requests.FPDFSignatureObj_GetSubFilter{
						Signature: signature,
					})
					Expect(err).To(BeNil())
				...
				It("returns no signature reason", func() {
					reason, err := pdfiumContainer.FPDFSignatureObj_GetReason(&requests.FPDFSignatureObj_GetReason{
						Signature: signature,
					})
					Expect(err).To(BeNil())
				...
				It("returns the correct signature time", func() {
					signatureTime, err := pdfiumContainer.FPDFSignatureObj_GetTime(&requests.FPDFSignatureObj_GetTime{
						Signature: signature,
					})
					Expect(err).To(BeNil())
				...
				It("returns no DocMDPPermission", func() {
					docMDPPermission, err := pdfiumContainer.FPDFSignatureObj_GetDocMDPPermission(&requests.FPDFSignatureObj_GetDocMDPPermission{
						Signature: signature,
					})
