	...
	Context("a PDF file with XFA data", func() {
		...
		When("is opened", func() {
			...
			It("returns the correct XFA packet content", func() {
				...
				Expect(err).To(BeNil())
				Expect(FPDF_GetXFAPacketContent).To(Not(BeNil()))
				Expect(FPDF_GetXFAPacketContent.Index).To(Equal(1))
				Expect(len(FPDF_GetXFAPacketContent.Content)).To(BeNumerically("~", 650, 100)) // Allow some room, not all platforms return the same value.
			})
