	...
	Describe("fpdf_doc", func() {
		Context("no document", func() {
			When("is opened", func() {
				...
				It("returns an error when calling FPDFAction_GetDest", func() {
					FPDFAction_GetDest, err := pdfiumContainer.FPDFAction_GetDest(&requests.FPDFAction_GetDest{})
					Expect(err).To(MatchError("document not given"))
					Expect(FPDFAction_GetDest).To(BeNil())
				})

				It("returns an error when calling FPDFAction_GetURIPath", func() {
					FPDFAction_GetURIPath, err := pdfiumContainer.FPDFAction_GetURIPath(&requests.FPDFAction_GetURIPath{})
					Expect(err).To(MatchError("document not given"))
					Expect(FPDFAction_GetURIPath).To(BeNil())
				})

				It("returns an error when calling FPDFDest_GetDestPageIndex", func() {
					FPDFDest_GetDestPageIndex, err := pdfiumContainer.FPDFDest_GetDestPageIndex(&requests.FPDFDest_GetDestPageIndex{})
					Expect(err).To(MatchError("document not given"))
					Expect(FPDFDest_GetDestPageIndex).To(BeNil())
				})

				It("returns an error when calling FPDF_GetFileIdentifier", func() {
					FPDF_GetFileIdentifier, err := pdfiumContainer.FPDF_GetFileIdentifier(&requests.FPDF_GetFileIdentifier{})
					Expect(err).To(MatchError("document not given"))
					Expect(FPDF_GetFileIdentifier).To(BeNil())
				})

				It("returns an error when calling FPDF_GetPageLabel", func() {
					FPDF_GetPageLabel, err := pdfiumContainer.FPDF_GetPageLabel(&requests.FPDF_GetPageLabel{})
					Expect(err).To(MatchError("document not given"))
					Expect(FPDF_GetPageLabel).To(BeNil())
				})

				It("returns an error when calling FPDFLink_GetDest", func() {
					FPDFLink_GetDest, err := pdfiumContainer.FPDFLink_GetDest(&requests.FPDFLink_GetDest{})
					Expect(err).To(MatchError("document not given"))
			...
			When("is opened", func() {
				...
				Context("without giving an action", func() {
					It("FPDFAction_GetURIPath returns an error", func() {
						uriPath, err := pdfiumContainer.FPDFAction_GetURIPath(&requests.FPDFAction_GetURIPath{
							Document: doc,
						})
					...
					It("FPDFAction_GetFilePath returns an error", func() {
						filePath, err := pdfiumContainer.FPDFAction_GetFilePath(&requests.FPDFAction_GetFilePath{})
						Expect(err).To(MatchError("action not given"))
						Expect(filePath).To(BeNil())
					})

					It("FPDFAction_GetDest returns an error", func() {
						dest, err := pdfiumContainer.FPDFAction_GetDest(&requests.FPDFAction_GetDest{
							Document: doc,
						})
						Expect(err).To(MatchError("action not given"))
					...
					It("FPDFAction_GetType returns an error", func() {
						actionType, err := pdfiumContainer.FPDFAction_GetType(&requests.FPDFAction_GetType{})
						Expect(err).To(MatchError("action not given"))
				...
				Context("without giving a dest", func() {
					It("FPDFDest_GetView returns an error", func() {
						destView, err := pdfiumContainer.FPDFDest_GetView(&requests.FPDFDest_GetView{})
						Expect(err).To(MatchError("dest not given"))
						Expect(destView).To(BeNil())
					})

					It("FPDFDest_GetDestPageIndex returns an error", func() {
						destPageIndex, err := pdfiumContainer.FPDFDest_GetDestPageIndex(&requests.FPDFDest_GetDestPageIndex{
							Document: doc,
						})
						Expect(err).To(MatchError("dest not given"))
					...
					It("FPDFDest_GetLocationInPage returns an error", func() {
						location, err := pdfiumContainer.FPDFDest_GetLocationInPage(&requests.FPDFDest_GetLocationInPage{})
						Expect(err).To(MatchError("dest not given"))
				...
				Context("without giving a page", func() {
					It("FPDFLink_GetLinkAtPoint returns an error", func() {
						FPDFLink_GetLinkAtPoint, err := pdfiumContainer.FPDFLink_GetLinkAtPoint(&requests.FPDFLink_GetLinkAtPoint{})
					...
					It("FPDFLink_GetLinkZOrderAtPoint returns an error", func() {
						FPDFLink_GetLinkZOrderAtPoint, err := pdfiumContainer.FPDFLink_GetLinkZOrderAtPoint(&requests.FPDFLink_GetLinkZOrderAtPoint{})
						Expect(err).To(MatchError("either page reference or index should be given"))
						Expect(FPDFLink_GetLinkZOrderAtPoint).To(BeNil())
					})

					It("FPDFLink_Enumerate returns an error", func() {
						FPDFLink_Enumerate, err := pdfiumContainer.FPDFLink_Enumerate(&requests.FPDFLink_Enumerate{})
						Expect(err).To(MatchError("either page reference or index should be given"))
						Expect(FPDFLink_Enumerate).To(BeNil())
					})

					It("FPDFLink_GetAnnot returns an error", func() {
						FPDFLink_GetAnnot, err := pdfiumContainer.FPDFLink_GetAnnot(&requests.FPDFLink_GetAnnot{})
						Expect(err).To(MatchError("either page reference or index should be given"))
						Expect(FPDFLink_GetAnnot).To(BeNil())
					})

					It("FPDF_GetPageAAction returns an error", func() {
						FPDF_GetPageAAction, err := pdfiumContainer.FPDF_GetPageAAction(&requests.FPDF_GetPageAAction{})
						Expect(err).To(MatchError("either page reference or index should be given"))
				...
				Context("without giving a link", func() {
					...
					It("FPDFLink_GetAction returns an error", func() {
						FPDFLink_GetAction, err := pdfiumContainer.FPDFLink_GetAction(&requests.FPDFLink_GetAction{})
						Expect(err).To(MatchError("link not given"))
					...
					It("FPDFLink_GetAnnotRect returns an error", func() {
						FPDFLink_GetAnnotRect, err := pdfiumContainer.FPDFLink_GetAnnotRect(&requests.FPDFLink_GetAnnotRect{})
						Expect(err).To(MatchError("link not given"))
						Expect(FPDFLink_GetAnnotRect).To(BeNil())
					})

					It("FPDFLink_CountQuadPoints returns an error", func() {
						FPDFLink_CountQuadPoints, err := pdfiumContainer.FPDFLink_CountQuadPoints(&requests.FPDFLink_CountQuadPoints{})
						Expect(err).To(MatchError("link not given"))
						Expect(FPDFLink_CountQuadPoints).To(BeNil())
					})

					It("FPDFLink_GetQuadPoints returns an error", func() {
						FPDFLink_GetQuadPoints, err := pdfiumContainer.FPDFLink_GetQuadPoints(&requests.FPDFLink_GetQuadPoints{})
						Expect(err).To(MatchError("link not given"))
		...
		Context("a PDF file with bookmarks", func() {
			...
			When("FPDFBookmark_GetDest is called", func() {
				It("it returns an error when no document is given", func() {
					action, err := pdfiumContainer.FPDFBookmark_GetDest(&requests.FPDFBookmark_GetDest{})
				...
				It("it returns no dest because the bookmark has none", func() {
					...
					dest, err := pdfiumContainer.FPDFBookmark_GetDest(&requests.FPDFBookmark_GetDest{
						Document: doc,
			...
			When("FPDFBookmark_GetAction is called", func() {
				It("it returns an error when no bookmark is given", func() {
					action, err := pdfiumContainer.FPDFBookmark_GetAction(&requests.FPDFBookmark_GetAction{})
		...
		Context("a PDF file with a link", func() {
			...
			When("looking for the link", func() {
				...
				It("FPDFLink_GetLinkZOrderAtPoint returns the z-order of the link", func() {
					pageLinkZOrder, err := pdfiumContainer.FPDFLink_GetLinkZOrderAtPoint(&requests.FPDFLink_GetLinkZOrderAtPoint{
						Page: requests.Page{
							ByIndex: &requests.PageByIndex{
								Document: doc,
								Index:    0,
		...
		Context("a PDF file with a launch action", func() {
			...
			When("FPDFLink_GetAction is called", func() {
				It("returns an action", func() {
					action, err := pdfiumContainer.FPDFLink_GetAction(&requests.FPDFLink_GetAction{
						Link: link,
					})
					Expect(err).To(BeNil())
			...
			When("FPDFLink_GetAnnot is called", func() {
				It("returns an annotation", func() {
					annotation, err := pdfiumContainer.FPDFLink_GetAnnot(&requests.FPDFLink_GetAnnot{
						Link: link,
						Page: requests.Page{
							ByIndex: &requests.PageByIndex{
								Document: doc,
			...
			When("FPDFLink_GetAnnotRect is called", func() {
				It("returns an annotation rect", func() {
					annotationRect, err := pdfiumContainer.FPDFLink_GetAnnotRect(&requests.FPDFLink_GetAnnotRect{
						Link: link,
					})
					Expect(err).To(BeNil())
			...
			When("FPDFLink_CountQuadPoints is called", func() {
				It("returns the quad points count of 0", func() {
					quadPointsCount, err := pdfiumContainer.FPDFLink_CountQuadPoints(&requests.FPDFLink_CountQuadPoints{
						Link: link,
					})
					Expect(err).To(BeNil())
			...
			When("FPDFLink_GetQuadPoints is called", func() {
				It("returns no quad points", func() {
					quadPoints, err := pdfiumContainer.FPDFLink_GetQuadPoints(&requests.FPDFLink_GetQuadPoints{
						Link:      link,
						QuadIndex: 0,
					})
					Expect(err).To(BeNil())
			...
			Context("A launch action is loaded", func() {
				...
				When("FPDFAction_GetFilePath is called", func() {
					It("returns a file path", func() {
						actionType, err := pdfiumContainer.FPDFAction_GetFilePath(&requests.FPDFAction_GetFilePath{
							Action: action,
						})
						Expect(err).To(BeNil())
						Expect(actionType).To(Not(BeNil()))
		...
		Context("a PDF file with a link dest", func() {
			...
			When("FPDFLink_GetDest is called", func() {
				It("returns a dest", func() {
					dest, err := pdfiumContainer.FPDFLink_GetDest(&requests.FPDFLink_GetDest{
						Document: doc,
						Link:     link,
					})
					Expect(err).To(BeNil())
			...
			When("a dest is loaded", func() {
				var dest references.FPDF_DEST
				BeforeEach(func() {
					destResp, err := pdfiumContainer.FPDFLink_GetDest(&requests.FPDFLink_GetDest{
						Document: doc,
						Link:     link,
					})
					Expect(err).To(BeNil())
				...
				When("FPDFDest_GetLocationInPage is called", func() {
					It("returns the right location", func() {
						FPDFDest_GetLocationInPage, err := pdfiumContainer.FPDFDest_GetLocationInPage(&requests.FPDFDest_GetLocationInPage{
							Dest: dest,
						})
						Expect(err).To(BeNil())
				...
				When("FPDFDest_GetView is called", func() {
					It("returns the right view", func() {
						FPDFDest_GetView, err := pdfiumContainer.FPDFDest_GetView(&requests.FPDFDest_GetView{
							Dest: dest,
						})
						Expect(err).To(BeNil())
