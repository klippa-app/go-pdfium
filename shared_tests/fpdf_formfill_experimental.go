...
var _ = Describe("fpdf_formfill_experimental", func() {
	...
	Context("a PDF file with a text form", func() {
		...
		renderFormImage := func(page references.FPDF_PAGE) {
			...
			os.WriteFile(TestDataPath+"/testdata/"+fmt.Sprintf("render_fpdf_formfill_experimental-%d.jpg", renderCount), imgBuf.Bytes(), 0777)
			renderCount++
	...
	Context("a PDF file with a form annotation", func() {
		...
		renderFormImage := func(page references.FPDF_PAGE) {
			...
			os.WriteFile(TestDataPath+"/testdata/"+fmt.Sprintf("render_fpdf_formfill_experimental_annotation-%d.jpg", renderCount), imgBuf.Bytes(), 0777)
			renderCount++
			//log.Println("did render")
		}
		...
		BeforeEach(func() {
			formHistory = []FormHistory{}
			pdfData, err := os.ReadFile(TestDataPath + "/testdata/annotiter.pdf")
			Expect(err).To(BeNil())
	...
	Context("a PDF file with a form annotation with color", func() {
		...
		BeforeEach(func() {
			formHistory = []FormHistory{}
			pdfData, err := os.ReadFile(TestDataPath + "/testdata/text_form_color.pdf")
			Expect(err).To(BeNil())
	...
	Context("a PDF file with a form combobox", func() {
		...
		renderFormImage := func(page references.FPDF_PAGE) {
			...
			os.WriteFile(TestDataPath+"/testdata/"+fmt.Sprintf("render_fpdf_formfill_experimental_combobox-%d.jpg", renderCount), imgBuf.Bytes(), 0777)
			renderCount++
			//log.Println("did render")
		}
	...
	Context("a PDF file with a form annotation with text color", func() {
		...
		BeforeEach(func() {
			formHistory = []FormHistory{}
			pdfData, err := os.ReadFile(TestDataPath + "/testdata/freetext_annotation_without_da.pdf")
			Expect(err).To(BeNil())
