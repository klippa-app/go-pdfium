...
func (p *PdfiumImplementation) FPDF_RenderPageBitmap_Start(request *requests.FPDF_RenderPageBitmap_Start) (*responses.FPDF_RenderPageBitmap_Start, error) {
	...
	res, err := p.call("IFSDK_PAUSE_Create", refPointer.Pointer)
	if err != nil {
		return nil, err
	}
	...
	res, err = p.call("FPDF_RenderPageBitmap_Start", *bitmapHandle.handle, *pageHandle.handle, *(*uint64)(unsafe.Pointer(&request.StartX)), *(*uint64)(unsafe.Pointer(&request.StartY)), *(*uint64)(unsafe.Pointer(&request.SizeX)), *(*uint64)(unsafe.Pointer(&request.SizeY)), *(*uint64)(unsafe.Pointer(&request.Rotate)), *(*uint64)(unsafe.Pointer(&request.Flags)), pausePointer)
	if err != nil {
		return nil, err
	}
...
func (p *PdfiumImplementation) FPDF_RenderPage_Continue(request *requests.FPDF_RenderPage_Continue) (*responses.FPDF_RenderPage_Continue, error) {
	...
	res, err := p.call("FPDF_RenderPage_Continue", *pageHandle.handle, pausePointer)
	if err != nil {
		return nil, err
	}
...
func (p *PdfiumImplementation) FPDF_RenderPage_Close(request *requests.FPDF_RenderPage_Close) (*responses.FPDF_RenderPage_Close, error) {
	...
	_, err = p.call("FPDF_RenderPage_Close", *pageHandle.handle)
	if err != nil {
		return nil, err
	}
