...
func (p *PdfiumImplementation) FPDF_SaveWithVersion(request *requests.FPDF_SaveWithVersion) (*responses.FPDF_SaveWithVersion, error) {
	...
	res, err := p.call("FPDF_FILEWRITE_Create")
	if err != nil {
		return nil, err
	}
	...
	if request.FileVersion == 0 {
		res, err = p.call("FPDF_SaveAsCopy", *documentHandle.handle, fileWriterPointer, uint64(request.Flags))
		if err != nil {
			return nil, err
		}
		success = *(*int32)(unsafe.Pointer(&res[0]))
	} else {
		res, err = p.call("FPDF_SaveWithVersion", *documentHandle.handle, fileWriterPointer, uint64(request.Flags), *(*uint64)(unsafe.Pointer(&request.FileVersion)))
		if err != nil {
			return nil, err
		}
		success = *(*int32)(unsafe.Pointer(&res[0]))
