...
func (p *PdfiumImplementation) getActionInfo(actionHandle *ActionHandle, documentHandle *DocumentHandle) (*responses.ActionInfo, error) {
	...
	if actionInfo.Type == enums.FPDF_ACTION_ACTION_GOTO {
		...
		_, err = p.Fn("FPDFAction_GetFilePath").Call(p.Context, *actionHandle.handle, charDataPointer.Pointer, uint64(filePathLength))
		if err != nil {
			return nil, err
		}
		...
		_, err = p.Fn("FPDFAction_GetURIPath").Call(p.Context, *documentHandle.handle, *actionHandle.handle, charDataPointer.Pointer, uint64(uriPathLength))
		if err != nil {
			return nil, err
		}
