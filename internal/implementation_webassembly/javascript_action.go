...
func (p *PdfiumImplementation) GetJavaScriptActions(request *requests.GetJavaScriptActions) (*responses.GetJavaScriptActions, error) {
	...
	for i := 0; i < javaScriptActionCount; i++ {
		res, err = p.Fn("FPDFDoc_GetJavaScriptAction").Call(p.Context, *documentHandle.handle, uint64(i))
		if err != nil {
			return nil, err
		}
		...
		defer p.Fn("FPDFDoc_CloseJavaScriptAction").Call(p.Context, javaScriptAction)

		// First get the name value length.
		res, err = p.Fn("FPDFJavaScriptAction_GetName").Call(p.Context, javaScriptAction, 0, 0)
		if err != nil {
			return nil, err
		}
		...
		_, err = p.Fn("FPDFJavaScriptAction_GetName").Call(p.Context, javaScriptAction, charDataPointer.Pointer, uint64(nameSize))
		if err != nil {
			return nil, err
		}
		...
		res, err = p.Fn("FPDFJavaScriptAction_GetScript").Call(p.Context, javaScriptAction, 0, 0)
		if err != nil {
			return nil, err
		}
		...
		_, err = p.Fn("FPDFJavaScriptAction_GetScript").Call(p.Context, javaScriptAction, charDataPointer.Pointer, uint64(scriptSize))
		if err != nil {
			return nil, err
		}
