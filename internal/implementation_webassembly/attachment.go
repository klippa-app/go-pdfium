...
func (p *PdfiumImplementation) GetAttachments(request *requests.GetAttachments) (*responses.GetAttachments, error) {
	...
	for i := 0; i < attachmentCount; i++ {
		res, err := p.Fn("FPDFDoc_GetAttachment").Call(p.Context, *documentHandle.handle, uint64(i))
		if err != nil {
			return nil, err
		}
		...
		res, err = p.Fn("FPDFAttachment_GetName").Call(p.Context, attachment, 0, 0)
		if err != nil {
			return nil, err
		}
		...
		_, err = p.Fn("FPDFAttachment_GetName").Call(p.Context, attachment, charDataPointer.Pointer, nameSize)
		if err != nil {
			return nil, err
		}
		...
		res, err = p.Fn("FPDFAttachment_GetFile").Call(p.Context, attachment, 0, 0, bufLenPointer.Pointer)
		if err != nil {
			return nil, err
		}
		...
		if int(getFileSizeSuccess) != 0 {
			...
			res, err = p.Fn("FPDFAttachment_GetFile").Call(p.Context, attachment, fileDataPointer.Pointer, bufLen, bufLenPointer.Pointer)
			if err != nil {
				return nil, err
			}
		...
		for _, requestKey := range requestKeys {
			...
			res, err = p.Fn("FPDFAttachment_GetValueType").Call(p.Context, attachment, keyStr.Pointer)
			if err != nil {
				return nil, err
			}
			...
			if newValue.ValueType == enums.FPDF_OBJECT_TYPE_STRING {
				...
				res, err = p.Fn("FPDFAttachment_GetStringValue").Call(p.Context, attachment, keyStr.Pointer, 0, 0)
				if err != nil {
					return nil, err
				}
				...
				res, err = p.Fn("FPDFAttachment_GetStringValue").Call(p.Context, attachment, keyStr.Pointer, charDataPointer.Pointer, uint64(stringValueSize))
				if err != nil {
					return nil, err
				}
