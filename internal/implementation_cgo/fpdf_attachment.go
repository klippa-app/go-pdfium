...
func (p *PdfiumImplementation) FPDFAttachment_SetDescription(request *requests.FPDFAttachment_SetDescription) (*responses.FPDFAttachment_SetDescription, error) {
	...
	success := C.FPDFAttachment_SetDescription(attachmentHandle.handle, (C.FPDF_WIDESTRING)(unsafe.Pointer(&transformedText[0])))
	if int(success) == 0 {
...
func (p *PdfiumImplementation) FPDFAttachment_GetDescription(request *requests.FPDFAttachment_GetDescription) (*responses.FPDFAttachment_GetDescription, error) {
	...
	descriptionSize := C.FPDFAttachment_GetDescription(attachmentHandle.handle, nil, 0)
	if descriptionSize == 0 {
		return nil, errors.New("could not get description")
	}

	charData := make([]byte, descriptionSize)
	C.FPDFAttachment_GetDescription(attachmentHandle.handle, (*C.ushort)(unsafe.Pointer(&charData[0])), C.ulong(len(charData)))
