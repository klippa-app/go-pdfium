...
// interpreterMode is set at init time from the WAZY_INTERPRETER environment
// variable. When true the whole suite uses wazy's interpreter engine instead
// of the default compiler engine.
var interpreterMode = os.Getenv("WAZY_INTERPRETER") == "1"

// runtimeConfig returns the wazero RuntimeConfig that matches the current
// mode (interpreter or compiler).
func runtimeConfig() wazyrt.RuntimeConfig {
	// The bundled PDFium module requires the exception handling core feature.
	features := wazyapi.CoreFeaturesV2 | wazyapi.CoreFeatureExceptionHandling
	if interpreterMode {
		return wazyrt.NewRuntimeConfigInterpreter().WithCoreFeatures(features)
	}
	return wazyrt.NewRuntimeConfig().WithCoreFeatures(features)
}
...
var _ = Describe("Wazy", func() {
	shared_tests.Import()

	Context("custom WASM", func() {
		It("requires a configured module", func() {
			pool, err := wazy.InitWithWASM(wazy.Config{})
	...
	Context("Kill", func() {
		...
		// makes wazy respect that cancellation during execution.
